...
function  LetterButtons({ text ,usedLetters, onLetterClick}){
    ...
    const selectLetters = new Set(usedLetters.join('').toUpperCase().split(''));
    const buttonStyle = function(letter){
        if(selectLetters.has(letter)){
            return `${originalCharacters.has(letter)?'bg-green-600 border-green-700':'border-[#000] border-4 bg-red-600'}`
        } else {
            return 'bg-blue-600 border-blue-700 hover:bg-blue-900'
        }
    }
    ...
    const buttons = ALPHABETS.map(letter=>{
        return(
            ...
            value={letter}
            disabled={selectLetters.has(letter)}
            onClick={handleClick}
            className={`h-12 w-12 m-1 rounded-md focus:outline-none text-white ${buttonStyle(letter)} `}>{letter}</button>
        )
    })
    return(
        <>
